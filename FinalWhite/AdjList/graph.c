...
void mergeSort(struct Graph *g, int l, int r){
  if(l<r){
    int m = l+(r-1)/2;
}
}
...
void swap(struct node **list, int a, int b){
 // printf("SWAPPING %d and %d\n",a,b);
  struct node *temp = list[a];
  list[a] = list[b];
  list[b] = temp;
}
int partition(struct node **list, int low, int high){
  //int piv = g->adjLists[high]->visited;
    int piv = (list[low]->visited + list[numNodes/2]->visited + list[high]->visited)/3;
...
int quicksort(struct node **list, int low, int high){
...
#pragma omp parallel
{
  quicksort(graph->adjLists, 0, numNodes-1);
} 
  printf("////////////////////\n\n");
  printGraph(graph);
  return 0;
